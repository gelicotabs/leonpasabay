...
const AuthProvider = ({ children }) => {
  ...
  const login = async ({ email, password, greet = true }) => {
    enqueueSnackbar("Login is currently disabled, proceeding as guest", { variant: "warning" });

  // Prepare dummy auth data (adjust as needed)
  const dummyData = {
    token: "guest-token", // a dummy token
    user: { name: "Guest", email, userGroup: 1 },
    message: "Logged in as guest"
  };

  // Save dummy token to localStorage and update state
  localStorage.setItem("token", dummyData.token);
  dispatch({
    type: "LOGIN_SUCCESS",
    payload: dummyData,
  });
  setAuthToken(dummyData.token);
